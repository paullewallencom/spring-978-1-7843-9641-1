...
public class CommunityServiceImplTest {
	...
	private static Set<Authority> ADMIN_AUTHORITIES = Sets.newHashSet(new Authority("ROLE_ADMIN"), new Authority("ROLE_OAUTH2"), new Authority("ROLE_BASIC"));
	
	@Before
	public void setup(){
		Set<Authority> authorities = Sets.newHashSet(new Authority("ROLE_OAUTH2"), new Authority("ROLE_BASIC"));
	...
	public void delete_methodSecured() throws NoSuchMethodException {
		...
		assertTrue(Arrays.asList(values).contains("ROLE_ADMIN"));
		assertTrue(Arrays.asList(values).contains("ROLE_SYSTEM"));
	}
	...
	public void signInUser_authTokenSetInContext() {
		...
		assertTrue(authArgumentCaptor.getValue().getAuthorities()
													.stream()
													.filter(a -> a.getAuthority().equals("ROLE_BASIC"))
													.findAny()
													.isPresent());
		
		assertTrue(authArgumentCaptor.getValue().getAuthorities()
													.stream()
													.filter(a -> a.getAuthority().equals("ROLE_OAUTH2"))
													.findAny()
													.isPresent());
	}
